...
function main() {
    ...
    T1 = timeSum * countSum + timeDiff * countDiff + timeMult * countMult + 
         timeDiv * countDiv + timeCompare * countCompare;
    Lsum = Tn;
    Ky = T1 / Tn;
    e = Ky / n;
    rang = p * q * m;
    ...
    var operationData = [["Sum", "Difference", "Multiplicity", "Division", "Comparing"], 
    [timeSum * countSum, timeDiff * countDiff, timeMult * countMult, timeDiv * countDiv, timeCompare * countCompare]];
    fillTable(operationData, 2, 5, "operationData");

    var executedData = [["T1", "Tn", "Ky", "e", "D", "Lavg" ,"Lsum"], 
...
function executeC() {
    ...
    for (var i = 0; i < p; i++) {
        cij[i] = [];
        for (var j = 0; j < q; j++) {
            cij[i][j] = notAndF(i, j) * (3 * matrixG[i][j] - 2) * matrixG[i][j] + (notOrD(i, j) + 
                        (4 * maxFandD(i, j) - 3 * notOrD(i, j)) * matrixG[i][j]) * (1 - matrixG[i][j]);
            countMult += 7;
            countDiff += 3;
            countSum += 2;
        }
    ...
    operationTime = (7 * timeMult + 3 * timeSum + 4 * timeDiff + 2 * (m - 1) * timeMult + 
                    3 * (timeDiff * (m + 1) + timeMult * (m - 1)) + timeCompare) * Math.ceil(p * q / n);
    Tn += operationTime;
    return cij;
}
...
function executeF() {
    ...
    for (var i = 0; i < p; i++) {
        ...
        for (var j = 0; j < q; j++) {
            fijk[i][j] = [];
            for (var k = 0; k < m; k++) {
                fijk[i][j][k] = deltaCalculation(matrixA[i][k], matrixB[k][j]) * (2 * matrixE[0][k] - 1) * matrixE[0][k] + 
                                deltaCalculation(matrixB[k][j], matrixA[i][k]) * (1 + (4 * deltaCalculation(matrixA[i][k], matrixB[k][j]) - 2) * matrixE[0][k]) * (1 - matrixE[0][k]);
                countMult += 7;
                countDiff += 3;
                countSum += 2;
            }
    ...
    operationTime = (7 * timeMult + 2 * timeSum + 6 * timeDiff + 3 * timeDiv) * Math.ceil(p * q * m / n);
    Tn += operationTime;
    return fijk;
}
